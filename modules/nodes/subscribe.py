...
class SubscribeExecutor(BaseNodeExecutor):
    ...
    async def execute(self):
        try:
            ...
            self.log('info', f"Subscribe Node started (Mode: {mode})", action='subscribe_node_start')
            ...
            for index, candidate in enumerate(candidates):
                ...
                self.log('info', f"[{index+1}/{len(candidates)}] Processing candidate: {candidate.title or entity_str}", action='process_candidate')

                if candidate.status in ['SUBSCRIBED', 'BANNED', 'REJECTED']:
                     self.log('info', f"Skipping {entity_str} (Status: {candidate.status})", action='skip_candidate')
                     continue
                ...
                try:
                    # Resolve Entity
                    # If we have username, use it. If not, use peer_id/access_hash input entity if possible, or skip.
                    entity = await self._resolve_candidate(candidate)
                    if not entity:
                        candidate.status = 'INVALID'
                        db.session.commit()
                        self.log('warning', f"Could not resolve entity for {entity_str}", action='resolve_error')
                        continue
                    ...
                    if mode == 'auto':
                        if not await self._check_filters(entity, min_subs, max_subs):
                            ...
                            db.session.commit()
                            self.log('info', f"Skipped {entity_str} by filter", action='filter_skip')
                            continue
                    ...
                    if status == 'JOINED':
                        ...
                        self.log('info', f"Request sent to {candidate.title} (Pending)", action='join_pending')
                    elif status == 'ALREADY_PARTICIPANT':
                        candidate.status = 'SUBSCRIBED'
                        candidate.subscribed_at = datetime.utcnow() # Update timestamp?
                        self.log('info', f"Already in {candidate.title}", action='join_already')
